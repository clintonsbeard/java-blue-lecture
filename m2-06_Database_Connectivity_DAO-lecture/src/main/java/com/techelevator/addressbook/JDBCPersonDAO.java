...
public class JDBCPersonDAO implements PersonDAO {
	...
	public void save(Person person) {
		...
		SqlRowSet results = jdbcTemplate.queryForRowSet(insertSql, person.getFirstName(), person.getLastName(),
							person.getDateAdded(), person.getType(), person.getUpdateDate());
	...
	private Person mapRowToPerson(SqlRowSet results) {
		...
		if (results.getDate("update_date") != null) {
			person.setDateAdded(results.getTimestamp("update_date").toLocalDateTime());
		}
